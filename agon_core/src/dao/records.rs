...
/// (`FootballScoreRecord` etc., defined further down this file). Adding a
/// sport here (plus writing its record types and a `SportRecord` impl in
/// `agon_core::sports`) is the only DAO-side change needed — no hand-editing
/// three separate enum declarations.
///
...
define_sport_records! {
    Football { score: FootballScoreRecord, format: FootballFormatRecord, live_event: FootballLiveEventRecord },
    Cricket { score: CricketScoreRecord, format: CricketFormatRecord, live_event: CricketLiveEventRecord },
    Netball { score: NetballScoreRecord, format: NetballFormatRecord, live_event: NetballLiveEventRecord },
}
